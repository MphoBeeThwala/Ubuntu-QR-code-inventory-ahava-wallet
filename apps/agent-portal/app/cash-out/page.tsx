...
export default function CashOutPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (cents > wallet.balanceCents) {
      setSubmitError(
        `Insufficient balance. Available: ${fmtZAR(wallet.balanceCents)}`,
      );
      return;
    }
    setSubmitError("");
    ...
    try {
      const res = await api.post("/agents/cash-out", {
  ...
  const amountCents = amountRands
    ? Math.round(parseFloat(amountRands) * 100)
    : 0;
  const exceedsBalance = wallet !== null && amountCents > wallet.balanceCents;
              ...
              <form onSubmit={handleLookup} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent font-mono"
                    required
                  />
                </div>
                {lookupError && (
                ...
                <button
                  ...
                  className="w-full bg-orange-600 hover:bg-orange-700 disabled:bg-gray-200 disabled:text-gray-400 text-white font-semibold py-3 rounded-xl transition text-sm"
                >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="0.00"
                        className={`w-full border rounded-xl px-4 py-3 pl-8 text-lg font-semibold focus:outline-none focus:ring-2 focus:border-transparent ${
                          exceedsBalance
                    ...
                    {amountRands && parseFloat(amountRands) > 0 && (
                      <p
                        className={`text-xs mt-1 ${exceedsBalance ? "text-red-500" : "text-gray-400"}`}
                      >
                        {exceedsBalance
                          ? `Exceeds available balance of ${fmtZAR(wallet.balanceCents)}`
                          : `Balance after: ${fmtZAR(wallet.balanceCents - amountCents)}`}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {[50, 100, 200, 500].map((amt) => (
                      <button
                        key={amt}
                        type="button"
                        onClick={() => setAmountRands(String(amt))}
                        className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg hover:border-orange-400 hover:text-orange-600 transition"
                      >
                        R{amt}
                      </button>
                    ))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Cash withdrawal request"
                  ...
                  <div className="flex gap-3 pt-1">
                    ...
                    <button
                      type="submit"
                      disabled={submitLoading || !amountRands || exceedsBalance}
                      className="flex-1 bg-orange-600 hover:bg-orange-700 disabled:bg-gray-200 disabled:text-gray-400 text-white font-semibold py-3 rounded-xl transition text-sm"
                    >
                      {submitLoading
                        ? "Processing…"
                        : `Pay Out ${amountRands ? fmtZAR(amountCents) : "cash"}`}
                    </button>
                  </div>
                </form>
              </div>
          ...
          {step === "success" && wallet && (
            <div className="bg-white rounded-2xl shadow-sm border border-green-100 p-8 text-center">
              ...
              <p className="text-gray-500 text-sm mb-6">
                {fmtZAR(amountCents)} paid out from {wallet.holderName}&apos;s
                wallet
              </p>
              {transactionId && (
              ...
              <div className="flex gap-3">
                <button
                  onClick={reset}
                  className="flex-1 bg-orange-600 hover:bg-orange-700 text-white font-semibold py-3 rounded-xl text-sm transition"
                >
                  New Cash Out
                </button>
                <button
          ...
          {step === "error" && (
            <div className="bg-white rounded-2xl shadow-sm border border-red-100 p-8 text-center">
              ...
              <button
                ...
                className="w-full bg-orange-600 hover:bg-orange-700 text-white font-semibold py-3 rounded-xl text-sm transition"
              >
                Try Again
              </button>
            </div>
