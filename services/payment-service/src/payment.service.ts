...
import { formatZAR, generateTransactionRef } from "./utils/format.utils";
...
export class PaymentService {
  ...
  async initiatePayment(
    ...
    if (dto.amountCents < 300) {
      throw new AhavaError(
        ...
        "Minimum payment amount is R3.00 (300 cents)",
      );
    }
    ...
    if (!recipientWallet) {
      throw new AhavaError(
        ...
        "Recipient wallet not found or inactive",
      );
    }
    ...
    const feeCents = calculateFee(
      dto.amountCents,
      dto.isFamilyTransfer ?? false,
    );
    ...
    const result = await this.prisma.$transaction(
      ...
      `;
        ...
        if (!lockedSender[0]) {
          throw new AhavaError(
            AhavaErrorCode.WAL_NOT_FOUND,
            "Sender wallet locked or not found",
          );
        }
        ...
        const lockedRecipient = await tx.$queryRaw<
          Array<{ balance: bigint; max_balance: bigint }>
        >`
      ...
      `;
        ...
        if (recipient && recipient.max_balance !== null) {
          const recipientNewBalance =
            Number(recipient.balance) + dto.amountCents;
        ...
        const newRecipientBalance =
          Number(recipient?.balance ?? 0) + dto.amountCents;
        ...
        const senderTxn = await tx.walletTransaction.create({
          data: {
            ...
            counterpartyName:
              recipientWallet.user.fullName ?? recipientWallet.walletNumber,
            ...
            ipAddress: dto.latitude
              ? `geo:${dto.latitude},${dto.longitude}`
              : undefined,
        ...
        await tx.walletTransaction.create({
          data: {
            ...
            counterpartyName:
              senderWallet.user?.fullName ?? senderWallet.walletNumber,
    ...
    await Promise.all([
      ...
      this.notificationQueue.addPaymentReceived({
        ...
        senderName:
          senderWallet.user?.preferredName ??
          senderWallet.user?.fullName ??
          "Someone",
        amountFormatted: formatZAR(dto.amountCents),
        reference: dto.reference,
        newBalanceFormatted: formatZAR(
          Number(recipientWallet.balance) + dto.amountCents,
        ),
      }),
  ...
  private async resolveRecipientWallet(dto: InitiatePaymentDto) {
    ...
    if (dto.recipientWalletNumber) {
      return this.prisma.wallet.findFirst({
        where: {
          walletNumber: dto.recipientWalletNumber,
          status: "ACTIVE",
          isDeleted: false,
        },
        include,
