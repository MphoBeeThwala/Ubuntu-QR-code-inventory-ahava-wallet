...
export class AuthService {
  ...
  async registerTier0(
    ...
    const pinHash = dto.pinHash
      ? await argon2.hash(dto.pinHash, ARGON2_OPTIONS)
      : null;
  ...
  async registerTier1(
    ...
    if (idExists) {
      throw new AhavaError(
        AhavaErrorCode.VAL_CONSTRAINT_VIOLATION,
  ...
  async login(dto: LoginDto, ipAddress: string): Promise<AuthResult> {
    ...
    const user = await this.prisma.user.findFirst({
      ...
      include: {
        wallets: { where: { walletType: "PERSONAL", isDeleted: false } },
      },
    });
    ...
    // Verify PIN
    const pinValid = user.pinHash
      ? await argon2.verify(user.pinHash, dto.pinHash)
      : false;
    ...
    if (!pinValid) {
      ...
      if (lockout) {
        throw new AhavaError(
          AhavaErrorCode.AUTH_PIN_LOCKED,
          `PIN locked for ${PIN_LOCKOUT_MINUTES} minutes after too many attempts`,
        );
      }
    ...
    if (user.primaryDeviceId && user.primaryDeviceId !== expectedDeviceHash) {
      ...
      this.logger.warn("Login from unbound device detected", {
        userId: user.id,
        deviceId: dto.deviceId,
      });
      // In production: send OTP to phone, require re-binding confirmation
      throw new AhavaError(
        AhavaErrorCode.AUTH_DEVICE_NOT_BOUND,
  ...
  async refreshTokens(
    ...
    if (stored.deviceId !== deviceHash) {
      ...
      await this.prisma.refreshToken.updateMany({
        ...
        data: {
          revokedAt: new Date(),
          revokedReason: "DEVICE_MISMATCH_SECURITY_REVOCATION",
        },
      });
      this.logger.error("Refresh token device mismatch — all tokens revoked", {
    ...
    return this.issueTokens(
      stored.userId,
      wallet.id,
      stored.user.kycTier as KycTier,
      deviceId,
      ipAddress,
    );
  }
