...
app.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { phoneNumber, pin, deviceId, deviceName, userAgent, ipAddress } =
        req.body;
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!user.pinHash || !(await verifyPin(pin, user.pinHash))) {
        ...
        const lockUntil =
          newFailedAttempts >= 5 ? new Date(Date.now() + 15 * 60 * 1000) : null; // Lock for 15min after 5 attempts
...
app.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Verify PIN
      if (!user.pinHash || !(await verifyPin(pin, user.pinHash))) {
        throw new AhavaError(AhavaErrorCode.AUTH_PIN_INCORRECT, "Invalid PIN", {
          requestId: req.id,
        });
      }
