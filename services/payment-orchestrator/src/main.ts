...
app.post("/orchestrate/payment", requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!resolvedRecipientId && recipientPhone) {
      const wallet = await prisma.wallet.findFirst({ where: { walletNumber: recipientPhone } });
...
export function startServer() {
  app.listen(PORT, () => {
    ...
    console.log(`Saga: http://localhost:${PORT}/orchestrate/payment`);
    console.log(`PayShap: http://localhost:${PORT}/orchestrate/payshap`);
  });
}
